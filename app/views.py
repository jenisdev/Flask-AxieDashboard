...
# Utils
from .util import edit_scholar, getAllCurrencies, getChangePercent, getRateForToken, getRateForSLP, Average_Gained_On_Date, add_scholar, Todays_Average_Gain, Yesterday_Average_Gain, Week_Average
...
def data():
    tabledata = db.session.query( 
        ...
        Scholarship.PersonalRoninAddress.label('personalroninaddress')
    )\
    .all()
    ...
    for row in tabledata:
        ...
        plain_row = [roninAddress, row[1], row[14], yesterday_gain, row[2], row[3],\
             row[4], row[5], row[6], row[7], row[8], row[9], row[10], row[11], row[12], [row[13], row[15], row[16], row[17]]]
